...
function Signin() {
  ...
  const navigate=useNavigate();
    ...
    const userHandler=async(e)=>
    {
      ...
      try{
        const response = await fetch("https://backend-server-pugh.onrender.com/api/signin",{method:"POST",headers:{'Content-Type':'application/json'},body:JSON.stringify(userDetails)});
        if(response.ok)
        {
    ...
    <>
      ...
      <form onSubmit={userHandler}>
        <div className="sub-cont">
          ...
          <div className="div-4">
            <button id='signin-btn-2' >Sign In</button>
          </div>
          <div className="div-5">
            OR
          </div>
          ...
          <div className="div-7">
            <span >New to Netflix? <button onClick={signuphandler} id="lst">Sign up now.</button></span>
          </div>
